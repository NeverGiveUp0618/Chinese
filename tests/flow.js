...
(async () => {
  ...
  ok(swText.includes("treasure-writing-v44") && swText.includes("essay-library.js") && !swText.includes("BAIBAI_AUDIO"), "★ 启动预缓存核心文件和原创作文库，白白语音按需缓存");
  ...
  ok(stops.length === 30, "30 个寻宝站点，分布在10条主题路线");
  ok(!!$("#scr-map .adventureMap") && $$("#scr-map .routeDot").length === 30, "★ 卡通探险路线图完整连接30站");
  ...
  ok($$("#scr-map .routeChapter").length === 10, "★ 十条主题路线可选");
  ok(!!$("#scr-map [data-route='history'] .historyMap") && !!$("#scr-map .historyRoad"), "★ 古都时光线改成有蜿蜒道路的漫画长卷地图");
  ok($$("#scr-map [data-route='history'] .historyStop").length === 3 && $$("#scr-map [data-route='history'] .historyDecor").length >= 5, "★ 古都路线分布在城门、古塔、河流和书院场景中");
  ok($$("#scr-map .comicMap").length === 10 && $$("#scr-map .comicRoad").length === 10, "★ 十条路线全部做成独立漫画地图");
  ok($$("#scr-map .mapCompass").length === 10 && $$("#scr-map .mapLegend").every(x => x.textContent.includes("方位关系参考真实方向")), "★ 十张地图都有北向标和方位边界说明");
  ok($$("#scr-map .wonderStop").length === 3 && $$("#scr-map .craftStop").length === 3, "★ 山河与匠心路线各有3个地图图钉关卡");
  ...
  ok(["桂林", "厦门", "成都", "北京", "洛阳", "景德镇", "武汉", "天津", "敦煌", "上海"].every(n => !cardOf(n).classList.contains("locked")), "★ 十条路线首站都默认解锁");
  ok(cardOf("三亚").classList.contains("locked"), "路线内的下一站仍需闯关解锁");
  cardOf("三亚").click();
  ...
  ok($$("#scr-passport .stamp").length === 30 && $$("#scr-passport .routeMedal").length === 10, "30枚城市章和10枚路线勋章都有位置");
